...
if len(sys.argv) < 2:
    print("Usage: py -3 t2_copy_files.py <destination folder> [<source folder>] [<excluded extension>]")
    print("If <source folder> is not provided, it defaults to the current working directory.")
...
if not os.path.exists(dest_path):
    print('Warning: non-existent destination path ({})'.format(dest_path))
    os.mkdir(dest_path)
    print('Folder {} has beed created.'.format(dest_path))

print('- - - - - - - - - - - - - - - - - - - - - - - - - - -')
print('Hello, ' + os.getlogin() + '!')
print('Directory to copy from:', source_path)
print('Directory to copy into:', dest_path)
print('Excluded file extension:', )
print('- - - - - - - - - - - - - - - - - - - - - - - - - - -')
